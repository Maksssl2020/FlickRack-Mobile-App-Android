...
import { MovieDataToManage, UserMovie } from "@/types/UserMovieTypes";

type SavedTmdbMovieCard = {
  movieData: UserMovie;
  onManageMovie?: (data: MovieDataToManage) => void;
};

const SavedTmdbMovieCard = ({
  movieData,
  onManageMovie,
}: SavedTmdbMovieCard) => {
  ...
  return (
    ...
    >
        ...
        <TouchableOpacity
          ...
          onPress={(event) => {
            ...
            onManageMovie?.({
              id: `${id}`,
              title: title,
              userMovieStatus: movieData.userMovieStatus,
            });
          }}
        >
      ...
      <View
        className={
          "absolute top-[50%] z-10 h-[35px] bg-custom-black-100 border-y-[3px] border-custom-violet-600 w-full flex items-center justify-center"
        }
      >
